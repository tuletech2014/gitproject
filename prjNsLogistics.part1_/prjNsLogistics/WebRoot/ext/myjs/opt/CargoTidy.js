Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : '../optBillInfoList.do?BIStateID=1001'
		}),
	...
	window.checkInfo = function() {
		...
		if (!win) {
			 ...
			 * 声明一个数组用于接受货物集合数组
			 */
			var cgarray = selectRecode.data['CargoIDs'];
			...
			var cgstore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : '../optCargoFindByID.do?BillID=' + srID
				}),
				reader : new Ext.data.JsonReader({
					totalProperty : 'totalProperty',
					root : 'root'
		 ...
		 * */
		from.form.load({
			url : '../optBillInfoFindByID.do?BillID=' + srID,
			method : 'POST'
		});
